...
class DB_Functions{
    ...
    public function insert_request($dt,$stime,$etime,$nos,$reason,$usremail)
    {
        ...
        $stmt = $this->conn->prepare("INSERT INTO Notification(Date,start_time,end_time,Nstudent,reason,requested_by) VALUES (?,?,?,?,?,?)");
    ...
    {
        $Ndetails = null;   
        $stmt=$this->conn->prepare("SELECT ID,Date,start_time,end_time,Nstudent,reason,requested_by FROM Notification");
    ...
    public function convey_requester($dt,$s_time,$req,$status)
    {
        $stmt=$this->conn->prepare("INSERT INTO notify_requester(Date,start_time,requested_by,status) values(?,?,?,?)");
        $stmt->bind_param("ssss",$dt,$s_time,$req,$status);
    ...
    public function fetch_notification($uname)
    {
        $stmt = $this->conn->prepare("SELECT Date, start_time,status FROM notify_requester WHERE requested_by=?");
        $stmt->bind_param("s",$uname);
        
        if($stmt->execute()){
            $stmt->bind_result($date, $s_time, $status);
            $i=0;
            if($stmt->fetch()==NULL)
            {
            ...
            while($stmt->fetch()){
                $notifyR[$i]["date"] = $date;
                $notifyR[$i]["s_time"] = $s_time;
                $notifyR[$i]["status"] = $status;
                $i++;   
            }
        }
        return $notifyR;
