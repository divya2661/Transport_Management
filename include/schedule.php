...
class schedule{
    ...
    public function CheckAvailability($BusSchedule,$count){
    	...
    	while ($i <2400 && $p<2400) { 
    		...
    		for ($j=8; $j < count($BusSchedule); $j++){


    			if($BusSchedule[$j]["GtoI"]["time"] < $i){

    				if($BusSchedule[$j]["GtoI"]["time"]==$i-30){
    					$PrivateCount = $PrivateCount -  $BusSchedule[$j]["GtoI"]["NumVehicle"];
    					// $availability[$k]["time"] = $i;
    					// $availability[$k]["available"] = $PrivateCount;
    					break;
    				}
    		...
    		for ($j=8; $j < count($BusSchedule); $j++){
    			...
    			if($BusSchedule[$j]["ItoG"]["time"]<$i){

    				if($BusSchedule[$j]["ItoG"]["time"]==$i-30){
    					$PrivateCount = $PrivateCount +  $BusSchedule[$j]["ItoG"]["NumVehicle"];
    					break;
    				}
    		...
    		$availability[$k]["time"] = $i;
    		$availability[$k]["available"] = $PrivateCount;
    		//uncomment to print availability table
    		//echo $availability[$k]["time"]." --> ".$availability[$k]["available"]."<br>";
    		$k++;

    		
    		for ($j=8; $j < count($BusSchedule); $j++) { 
    			
    			if($BusSchedule[$j]["GtoI"]["time"]<$p){

    				if($BusSchedule[$j]["GtoI"]["time"]==$p-70){
    					$PrivateCount = $PrivateCount -  $BusSchedule[$j]["GtoI"]["NumVehicle"];
    					// $availability[$k]["time"] = $p;
    					// $availability[$k]["available"] = $PrivateCount;
    					break;
    				}
    		...
    		for ($j=8; $j < count($BusSchedule); $j++){
    			...
    			if($BusSchedule[$j]["ItoG"]["time"]<$p){

    				if($BusSchedule[$j]["ItoG"]["time"]==$p-70){
    					$PrivateCount = $PrivateCount +  $BusSchedule[$j]["ItoG"]["NumVehicle"];
    					break;
    				}
